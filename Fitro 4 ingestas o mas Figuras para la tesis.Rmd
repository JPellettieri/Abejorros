...
SupPeso <- Datos %>%
  filter(!is.na(Muere))
SupPeso$CualiPeso <- as.factor(SupPeso$CualiPeso)
# Ajustar modelo de regresión logística
MSupPeso <- glm(Muere ~ CualiPeso, data = SupPeso, family = binomial)
...
```{r ModSuptrat sin agrupar, echo=TRUE}
# Filtrar los datos: solo individuos con al menos 2 ingestas
Datos_filtrado <- Datos %>%
  filter(Ingestas >= 2)

# Calcular índice de supervivencia por tratamiento (sin agrupar)
...
SupPeso$CualiPeso <- as.factor(SupPeso$CualiPeso)
# Ajustar modelo de regresión logística
MSupPeso <- glm(Muere ~ Tratamiento , data = SupPeso, family = binomial)
...
# Contrastes por pares si hay más de 2 niveles
library(emmeans)
emmeans(MSupPeso, pairwise ~ Tratamiento, type = "response")
...
modeloGen <- glm(Generaliza ~ Tratamiento, family = binomial, data = Datos_bin)
