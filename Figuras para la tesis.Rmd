...
Vivos <- Datos %>%
  filter( Muere == 0, Tratamiento != "SIN OLOR")
...
DatosMemoria <- Datos %>%
  filter(Tratamiento != "SIN OLOR", Nido != "4", Ingestas > 1, Muere == 0) %>%
  group_by(Tratamiento) %>%
  summarise(prob = mean(Recuerda) * 100, n = n())

ggplot(DatosMemoria, aes(x = Tratamiento, y = prob)) +
  geom_point(size = 3) +
  labs(x = "Tratamiento", y = "Probabilidad ajustada de memoria") +
  theme_minimal()
