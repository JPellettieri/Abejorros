...
MIng <- glmer(Recuerda ~ Ingestas + (1|Tratamiento)+ (1|Nido), 
              data = Vivos, 
              family = binomial,
...
MAprePeso <-glmer(A ~ as.factor(CualiPeso) + (1|Tratamiento) + (1|Nido), 
...
# Ajustar modelo logístico
MRecPeso <- glmer(Recuerda ~ as.factor(CualiPeso)+(1|Nido), data = Datos, family = binomial)
summary(MRecPeso)  
anova(MRecPeso)    #N/S
...
ggplot(em_means_df, aes(x = CualiPeso, y = Respuesta)) +
  ...
  labs(
    ...
    title = "Respuesta según recil de peso"
  ) +
  theme_minimal()
